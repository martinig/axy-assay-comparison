...
setwd("~/Documents/ECOL 530/Data Analysis/CSV")
...
assays<-read.csv("Trials.csv", header=T) %>%
	bind_rows(read.csv("~/Documents/ECOL 530/Data Analysis/CSV/Personality KRSP Master File (Dec 12, 2023).csv", header=T)%>%select(-X) %>%mutate_at(vars(walk, jump, hole, hang, chew, groom, still, front, back, attack, attacklatency, approachlatency), as.numeric)) %>% #Ben's master dataset has a lot of problems, so I have to fix them below

...
(adult_axy_all) %>% as_tibble() %>% dplyr::count(squirrel_id) %>% nrow() #255 adults
(adult_axy_all) %>% as_tibble() %>% dplyr::count(squirrel_id, axy_yr, axy_month) %>% nrow() #~640 sessions
nrow(adult_axy_all) #28546
